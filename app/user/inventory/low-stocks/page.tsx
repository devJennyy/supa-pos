...
export default function Page({ data = MOCK }: { data?: StockItem[] }) {
    ...
    <Table className="rounded-lg border overflow-hidden">
      ...
      <TableBody>
        {filtered.map((item) => {
          ...
          return (
            <TableRow key={item.id} className={classNames(st.label !== "Ok" && "bg-input/20")}>
              ...
              <TableCell>
                <Badge className={`${toneBadge(st.tone)} opacity-90 rounded-sm`}>
                  {st.label}
                </Badge>
              </TableCell>
  ...
  const renderAccordion = () => (
    <div className="space-y-3">
      ...
      {filtered.length === 0 && (
        <p className="text-center text-muted-foreground py-6">No items match your filters.</p>
      )}
    </div>
  );
    ...
    <main className="flex flex-1 flex-col gap-4 lg:p-5 p-4">
      {isLoading ? (
        ...
        <>
          ...
          <div className="block md:hidden">{renderAccordion()}</div>
        </>
      )}
    </main>
  );
